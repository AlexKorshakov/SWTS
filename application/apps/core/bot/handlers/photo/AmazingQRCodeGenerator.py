...
from apps.core.bot.handlers.photo.amzqr.mylibs import theqrmodule


async def create_qr_code_with_param(chat_id: str | int, act_number: str | int, qr_report_path: str):
    """Создание qr-кода

    :param chat_id:
    :param act_number:
    :param qr_report_path:
    :return:
    """
    tempdir = os.path.join(os.path.expanduser('~'), '.myqr')
    try:
        await create_qr_code(
            words=f'qr_act_nom_{chat_id}_{act_number}',
            save_name=f'qr_act_nom_{chat_id}_{act_number}.jpg',
            version=3,
            save_dir=qr_report_path
        )

    except ValueError as err:
        logger.error(f'{chat_id = } create_qr_code_with_param: {repr(err)}')
...
async def create_qr_code(words, version=1, level='H', picture=None, colorized=False,
    ...
    async def combine(ver, qr_name, bg_name, colorized, contrast, brightness, save_dir, save_name=None):
