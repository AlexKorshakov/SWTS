...
from apps.core.bot.handlers.photo.amzqr.mylibs.constant import required_remainder_bits, lindex, grouping_list
...
def structure_final_bits(ver, ecl, data_codewords, ecc):
    ...
    final_bits = ''.join(['0' * (8 - len(i)) + i for i in [bin(i)[2:] for i in final_message]]) + '0' * \
                 required_remainder_bits[ver - 1]
