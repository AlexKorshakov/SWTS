...
async def simple_meaning_handler(hse_user_id: int | str, character: str, item_number: int | str,
...
async def text_processor_character_text(table_dataframe: DataFrame, character: str, item_number_text: str | int,
    ...
    sub_character_text: str = await get_item_title_for_id(
        table_name=character_table_name, item_id=violations_dataframe[character].values[0]
    )
