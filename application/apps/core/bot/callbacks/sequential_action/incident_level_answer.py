...
@MyBot.dp.callback_query_handler(lambda call: call.data in get_names_from_json("INCIDENT_LEVEL"))
async def incident_level(call: types.CallbackQuery):
    """Обработка ответов содержащихся в INCIDENT_LEVEL
    """
    for i in get_names_from_json("INCIDENT_LEVEL"):
        try:
            if call.data == i:
                ...
                menu_list = board_config.menu_list = get_names_from_json("ACT_REQUIRED")
