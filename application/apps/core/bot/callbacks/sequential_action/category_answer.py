...
@MyBot.dp.callback_query_handler(lambda call: call.data in get_names_from_json("CATEGORY"))
async def category_answer(call: types.CallbackQuery):
    """Обработка ответов содержащихся в CATEGORY_LIST
    """
    for i in get_names_from_json("CATEGORY"):
        try:
            if call.data == i:
                ...
                menu_list = board_config.menu_list = get_names_from_json("VIOLATION_CATEGORY")
